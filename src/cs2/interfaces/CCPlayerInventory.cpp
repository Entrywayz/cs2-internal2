...
CCSInventoryManager* CCSInventoryManager::GetInstance() {
    ...
    static FnGetInstanceS oGetInstanceS = reinterpret_cast<FnGetInstanceS>(M::GetAbsoluteAddress(M::FindPattern(L"client.dll", "E8 ? ? ? ? 48 8B D8 8B F7"), 0x1));
...
CGCClientSystem* CGCClientSystem::GetInstance() {
    ...
    static FnGetInstanceSs fnGetClientSystem =
        reinterpret_cast<FnGetInstanceSs>(
            M::GetAbsoluteAddress(
                M::FindPattern(L"client.dll", "E8 ? ? ? ? 48 8B C8 E8 ? ? ? ? 8B D8 85 C0 74 33"),
                0x1, 0x0
            )
            );

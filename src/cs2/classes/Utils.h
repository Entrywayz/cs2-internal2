...
namespace Utils
{
    ...
    inline void NormalizeAngles(Vec3& a)
    {
    ...
    inline float GetFoV(const Vec3& view, const Vec3& aim)
    {
        Vec3 delta = aim - view;
    ...
    inline void AngleVec3s(const Vec3& ang, Vec3& forward)
    {
    ...
    inline Vec3 GetBonePos(C_CSPlayerPawn* pawn, BoneID boneID)
    {
        if (!pawn)
            return {};
        ...
        uintptr_t boneArray = *reinterpret_cast<uintptr_t*>(reinterpret_cast<uintptr_t>(scene) + cs2_dumper::schemas::client_dll::CSkeletonInstance::m_modelState + 0x80);
        if (!IsValidPtr(boneArray))
            return {};

        return *reinterpret_cast<Vec3*>(boneArray + static_cast<int>(boneID) * 0x20);
    }

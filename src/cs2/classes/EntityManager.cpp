...
EntityManager::EntityManager()
{
    ...
    entityListAddress = client ? client + cs2_dumper::offsets::client_dll::dwEntityList : 0;
}
...
void EntityManager::Update()
{
    ...
    if (client)
    {
        uintptr_t localPawnAddr = *reinterpret_cast<uintptr_t*>(client + cs2_dumper::offsets::client_dll::dwLocalPlayerPawn);
    ...
    {
        uintptr_t listEntry =
            *reinterpret_cast<uintptr_t*>(listPtr + (8 * (i >> 9) + 16));
        if (!listEntry)
            continue;

...
#include "../utils/module/modules.hpp"
class c_base_entity;
...
public:
	...
	c_cs_player_pawn* get_local_pawn() {
		static auto fn = reinterpret_cast<c_cs_player_pawn * (__fastcall*)(int)>(g_opcodes->scan_absolute(g_modules->m_modules.client_dll.get_name(), "E8 ? ? ? ? 48 8B F0 48 85 C0 74 ? 48 8D 15 ? ? ? ? B9", 0x1));
		return fn(0);
	}
